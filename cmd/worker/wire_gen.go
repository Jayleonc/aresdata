...
func wireApp(bootstrap *conf.Bootstrap, confData *conf.Data, logger log.Logger) (*App, func(), error) {
	...
	headlessUsecase := biz.NewHeadlessUsecase(sourceDataRepo, headlessFetcher, logger)
	fetchVideoTrendHeadlessTask := task.NewFetchVideoTrendHeadlessTask(headlessUsecase, videoUsecase, logger)
	v := task.NewTaskSet(fetchVideoRankTask, processVideoRankTask, fetchVideoTrendTask, processVideoTrendTask, fetchVideoSummaryTask, processVideoSummaryTask, fetchVideoTrendHeadlessTask)
	app := newApp(logger, v)
	return app, func() {
		cleanup()
	}, nil
